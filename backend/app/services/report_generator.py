...
class ReportGeneratorService:
    ...
    def generate_pdf(self, preview: ReportPreviewResponse, report_id: str) -> bytes:
        ...
        # 6. AI Investigation Insights Section (where available)
        if preview.ai_insights:
            story.append(Paragraph("5. AI Investigation Insights (Root Cause Hypotheses)", h2_style))
            story.append(Paragraph(
                "<b>Notice:</b> The following insights are generated by AI based strictly on structured evidence packets. "
                "They represent diagnostic explanations and recommended verification steps for finance operations.",
                callout_style
            ))
            story.append(Spacer(1, 4))
            ...
            for ins in preview.ai_insights[:2]:
                ai_box = [
                    [
                        ...
                        Paragraph(f"<b>AI Confidence:</b> {int(ins.get('confidence', 0.9)*100)}%", body_style)
                    ],
                    [
