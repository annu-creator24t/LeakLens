...
export function SkeletonMetricCard() {
  return (
    <div className="p-5 rounded-xl border border-slate-800/80 bg-[#0a0e17]/80 space-y-3">
...
export function SkeletonTable({ rows = 5, cols = 6 }: { rows?: number; cols?: number }) {
  return (
    <div className="rounded-xl border border-slate-800/80 bg-[#0a0e17]/80 overflow-hidden divide-y divide-slate-800/60">
      <div className="p-4 bg-slate-900/40 flex items-center space-x-4">
        {Array.from({ length: cols }).map((_, i) => (
          <SkeletonBox key={`th-${i}`} className={`h-4 flex-1 ${i === 0 ? "w-1/4" : ""}`} />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, r) => (
        <div key={`tr-${r}`} className="p-4 flex items-center space-x-4">
          {Array.from({ length: cols }).map((_, c) => (
            <SkeletonBox key={`td-${r}-${c}`} className={`h-4 flex-1 ${c === 0 ? "w-1/4" : ""}`} />
          ))}
        </div>
      ))}
    </div>
