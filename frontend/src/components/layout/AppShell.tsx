        ...
        <div>
          ...
          <div className="p-5 border-b border-slate-800/80 flex items-center justify-between">
            <Link href="/dashboard" className="flex items-center space-x-2.5">
              <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white font-bold shadow-[0_0_15px_rgba(37,99,235,0.4)]">
                LL
              </div>
              <div>
                <span className="font-bold text-sm tracking-tight text-white block">LEAKLENS</span>
                <span className="text-[10px] text-slate-400 font-mono">Financial Intelligence</span>
              </div>
            </Link>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-3 px-3.5 py-2.5 rounded-lg text-xs font-medium transition-all ${
                    isActive
                      ? "bg-blue-600/15 text-blue-400 border border-blue-600/30 font-semibold shadow-[0_0_12px_rgba(37,99,235,0.15)]"
        ...
        <div className="p-4 border-t border-slate-800/80 space-y-3">
          <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-800 space-y-2">
            ...
            <button
              ...
              className="w-full py-1.5 rounded bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-medium flex items-center justify-center space-x-1.5 transition-colors cursor-pointer"
            >
              <Activity className={`w-3 h-3 ${loadingDemo ? "animate-spin" : ""}`} />
              <span>{loadingDemo ? "Generating 10k..." : "Load 10k Benchmark"}</span>
            </button>
          </div>
        ...
        <header className="h-16 border-b border-slate-800/80 bg-[#080b11]/90 backdrop-blur-md sticky top-0 z-40 px-6 flex items-center justify-between">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-2.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 hover:border-slate-700 text-xs text-slate-200 transition-colors"
            >
              <Database className="w-3.5 h-3.5 text-blue-400" />
              <span className="text-slate-400">Dataset:</span>
              <span className="font-mono font-semibold text-white">
                {selectedDataset || "Select Session"}
              </span>
              <ChevronDown className="w-3 h-3 text-slate-500" />
            </button>

            {isDatasetOpen && (
              <div className="absolute left-0 top-full mt-2 w-72 bg-[#0c121e] border border-slate-800 rounded-xl shadow-2xl p-2 z-50 space-y-1">
                <div className="text-[10px] uppercase font-mono text-slate-400 px-2 py-1">
                  Active Financial Datasets
                </div>
                {datasets.length === 0 ? (
                  <div className="p-3 text-center text-xs text-slate-500 font-mono">
                    No sessions found. Load 10k demo or upload CSVs.
                  </div>
                ) : (
                  datasets.map((d) => (
                    <button
                    ...
                    >
                      <div className="truncate">
                        ...
                        <p className="text-[10px] font-mono text-slate-500">{d.dataset_id}</p>
                      </div>
          ...
          <div className="flex items-center space-x-3">
            <Link
              ...
              className="text-xs text-rose-400 hover:text-rose-300 border border-rose-900/50 px-3 py-1.5 rounded-lg bg-rose-950/40 flex items-center space-x-1.5"
            >
            ...
            <Link
              href="/upload"
              className="text-xs text-slate-300 hover:text-white border border-slate-800 bg-slate-900 px-3 py-1.5 rounded-lg flex items-center space-x-1.5 transition-colors"
            >
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#080b11] flex items-center justify-center text-slate-500">Loading LeakLens Shell...</div>}>
